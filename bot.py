...
log = logging.getLogger("sigmoji")

TOKEN      = os.getenv("DISCORD_TOKEN", "")
PORT       = int(os.getenv("PORT", "8080"))
START_TIME = time.time()

# Optional: register commands instantly to a specific guild while testing.
# Remove (or leave blank) for global registration in production.
...
# ── Intents ───────────────────────────────────────────────────────────────────
# message_content is a PRIVILEGED intent — you must also enable it in the
# Discord Developer Portal (Bot → Privileged Gateway Intents).
intents                 = discord.Intents.default()
intents.message_content = True   # Required to read plain-text answers in chat
...
<head>
  ...
  <title>Sigmoji — Bot Status</title>
  <style>
...
async def on_application_command_error(ctx: discord.ApplicationContext, error):
    log.error("Command error in /%s: %s", getattr(ctx.command, "name", "?"), error, exc_info=True)
    try:
